...
| --- | --- | --- |
| Request Identity Link | **Yes** | без numeric user id redemption не сопоставить с покупателем, и у зрителя нет кошелька GTA$ ([§11](#11-gta-dollar-gta)) |
...
Точный контракт — имена, схема, эндпоинты, коды ошибок, тексты интерфейса,
тесты — в [docs/GTA_DOLLAR_ECONOMY.md](docs/GTA_DOLLAR_ECONOMY.md). Этот
раздел — как этим пользоваться.

### Что это

Twitch не даёт расширению списать баллы зрителя: потратить их можно только в
окне наград самого Twitch. Поэтому точки покупаются за внутреннюю валюту.

* **ETH** — так на этом канале называются настоящие баллы канала (Channel
  Points) Twitch.
* **GTA DOLLAR**, в интерфейсе **`GTA$`** — целый баланс зрителя в нашей
  PostgreSQL. Twitch о нём ничего не знает, обратного обмена в ETH нет.
* Курс по умолчанию **1 ETH = 10 GTA$**. GTA$ покупаются одной настоящей
  наградой **«Обмен ETH на GTA DOLLAR»**: по умолчанию 500 ETH → 5 000 GTA$.
* Точку зритель оплачивает в GTA$ прямо в карте, одной кнопкой. Формула цены
  прежняя, меняется только единица: 1,4 км → GTA$ 1 500.

Каждое изменение баланса — строка в журнале, который только дописывается;
баланс кошелька всегда равен сумме его строк. Кошелёк привязан к числовому
Twitch user id — тому же, что приходит в EventSub. Поэтому зритель один раз
нажимает **ПОДКЛЮЧИТЬ** в карте (Identity Link), а анонимный — входит в
Twitch. GTA$ за обмен, сделанный до подключения, не теряются: они уже лежат на
этом user id и видны сразу после подключения.

### Награда обмена

Она одна на канал, и её создаёт и подтверждает **наш бэкенд** — тем же
приложением (`TWITCH_CLIENT_ID`) и тем же broadcaster OAuth, что и слоты:
при старте api (при `REAL_TWITCH=true` и подключённом канале), после broadcaster
OAuth и по кнопке **СИНХР. НАГРАДУ ОБМЕНА** в `/admin`. Новых scopes и подписок
EventSub не нужно: redemption обмена приходит по той же подписке `.add`, на
мгновение появляется в очереди запросов наград и сразу становится выполненным.

Отметить redemption выполненным Twitch разрешает только приложению, которое
создало награду. Отсюда правила:

* **Не создавай, не удаляй и не пересоздавай** награду обмена руками в панели
  Twitch. Ручную награду бэкенд не подтверждает и не считает своей: зритель
  отдаст ETH, а GTA$ не получит. Вдобавок она занимает название, и свою
  бэкенд создать уже не сможет — Twitch не пускает два одинаковых названия.
  Если так вышло: удалить ручную награду и нажать СИНХР. НАГРАДУ ОБМЕНА.
* **Не правь её на Twitch** — ни цену, ни название, ни описание, ни
  включённость. GTA$ всё равно считаются от того, что Twitch реально списал,
  но карта до следующей сверки обещает зрителю старую цену, а сверка (старт
  api, СИНХР. НАГРАДУ ОБМЕНА) вернёт всё как в настройках. Цену и курс меняй
  только в `/admin`.
* **Не отклоняй redemption обмена** в очереди запросов Twitch: ETH вернутся
  зрителю, а GTA$ останутся у него (см. ниже).

Слоты `IRL WAYPOINT 01…N` в режиме `gta_dollar` остаются на Twitch
выключенными и не удаляются. Redemption любых других наград канала сервер
игнорирует: не подтверждает и не возвращает.

### Курс и цена: `/admin` → GTA DOLLAR ECONOMY

`https://localhost:8080/admin.html` с этого компьютера или
`https://gudinigta6.duckdns.org/admin`, панель **GTA DOLLAR ECONOMY**:

* курс `1 ETH = 10 GTA$` и режим оплаты: «ОПЛАТА В GTA$» или «ОПЛАТА
  НАГРАДАМИ · LEGACY»;
* награда обмена на Twitch: название, цена, сколько GTA$ даёт один обмен,
  включена ли;
* оборот: «В обращении», «Кошельков», «Журнал» (сходится / РАСХОЖДЕНИЕ),
  «Выпущено», «Потрачено», «Возвращено», «Корректировки», последние 20
  операций;
* подтверждения на Twitch: «Ждут FULFILLED», «Сбой FULFILLED», «Отменены на
  Twitch»;
* поля **«GTA$ за 1 ETH»** (1…1000) и **«Цена награды, ETH»** (1…1 000 000),
  кнопка СОХРАНИТЬ.

Цены точек (панель «Настройки канала», группа «Цена») в режиме `gta_dollar` —
в GTA$, а не в ETH: поле так и подписано, «База, GTA$», и предпросмотр цены
тоже в GTA$. При курсе 10 «База 100» — это 10 ETH.

Новая цена при сохранении **сначала уходит на Twitch**; если Twitch отказал,
настройка не сохраняется и панель показывает ошибку. Отдельно синхронизировать
ничего не нужно. Новый курс переписывает описание награды на Twitch (курс
указан там; если Twitch в этот момент не ответил, курс всё равно сохраняется,
а описание поправит следующая сверка) и действует на обмены, обработанные
после сохранения: GTA$ — это сколько ETH Twitch списал, умноженное на курс в
момент обработки. Зритель видит курс и цену в карте, в диалоге
**+ ПОПОЛНИТЬ**: интерфейс берёт их с сервера, а не из кода.

СИНХР. НАГРАДУ ОБМЕНА создаёт награду, если её нет (например, её удалили на
Twitch), и возвращает ей цену, название, описание и включённость из настроек.
Пока награды нет, панель пишет «награда не создана», а диалог у зрителя —
«Награда обмена сейчас недоступна на Twitch». Если кнопка не помогает —
проверь broadcaster OAuth.

### WAYPOINT_PAYMENT_MODE

```
WAYPOINT_PAYMENT_MODE=gta_dollar             # по умолчанию: точки за GTA$
WAYPOINT_PAYMENT_MODE=channel_points_reward  # прежний режим: своя награда на каждую точку
```

Если переменной в `.env` нет, действует `gta_dollar`. Читается один раз при
старте api: после правки `.env` — `docker compose up -d --force-recreate api`.
В режиме `gta_dollar` старое подтверждение точки через слот отвечает
`payment_mode`.

**Откат на награды за каждую точку:**

1. Лучше между заданиями. Активную точку за GTA$ можно отменить с возвратом
   GTA$ и после отката.
2. В `.env`: `WAYPOINT_PAYMENT_MODE=channel_points_reward`.
3. `docker compose up -d --force-recreate api`.
4. В `/admin` проверить слоты; если что-то не так — «СИНХР. СЛОТЫ».

Новый zip для отката не нужен: оверлей узнаёт режим от сервера и сам
возвращается к прежней карточке. Балансы и журнал GTA$ сохраняются. Награда
обмена остаётся на Twitch и по-прежнему начисляет GTA$, но кошелька в карте в
этом режиме не видно и тратить GTA$ негде — предупреди зрителей не менять ETH,
пока не вернёшь `gta_dollar` (той же правкой `.env` и пересозданием api).

### Возвраты

* На телефоне стримера рядом с **ЗАВЕРШИТЬ** две кнопки — **НЕ МОГУ** и
  **НЕБЕЗОПАСНО**, обе нажимаются дважды, как и завершение. Каждая отменяет
  задание и возвращает покупателю **GTA$** — ровно цену точки; у него в карте
  всплывает «Возврат за задание» и новый баланс, без перезагрузки.
* `/admin` → ОТМЕНИТЬ с галочкой «Вернуть GTA$ зрителю» делает то же самое.
* `/admin` → **СБРОСИТЬ МАРШРУТ** (аварийный сброс) у точки за GTA$ тоже
  возвращает зрителю её цену, всегда: сброс не должен оставить его и без
  задания, и без GTA$. Отменить точку за GTA$ без возврата можно только через
  ОТМЕНИТЬ со снятой галочкой. Точку за баллы сброс, как и раньше, закрывает,
  не трогая Twitch.
* **ETH не возвращаются никогда**: обмен уже состоялся, его redemption
  подтверждён. Channel Points для точек за GTA$ сервер не трогает вовсе.
* **ЗАВЕРШИТЬ** (в контракте — «ДОШЁЛ»: стример дошёл, задание выполнено)
  ничего не возвращает — ни на телефоне, ни в `/admin`.
* Одна точка — не больше одного возврата. Это гарантирует база, а не только
  код: повторная отмена ничего не добавит.

Точку, купленную ещё прежней наградой (`channel_points_reward`), НЕ МОГУ и
НЕБЕЗОПАСНО просто отменяют: баллы за неё Twitch уже списал окончательно.

### Если Twitch не принял подтверждение

Обмен обрабатывается так: сначала одна транзакция в базе — запись в журнал и
+GTA$ на кошелёк, — и только после неё redemption отмечается на Twitch
выполненным. Зритель получает GTA$ сразу, даже если Twitch в эту минуту не
отвечает.

* **Twitch ответил ошибкой или не ответил.** Операция остаётся `PENDING`
  («Ждут FULFILLED»), сервер повторяет подтверждение каждые 60 секунд, до 10
  попыток. Redemption всё это время висит в очереди запросов наград.
* **10 попыток не помогли.** Операция становится `FAILED`, растёт «Сбой
  FULFILLED». GTA$ у зрителя остаются. Проверь в `/admin` broadcaster OAuth и
  закрой redemption в очереди Twitch руками — только «выполнено», не
  «отклонить».
* **Twitch ответил 400/404.** Сервер читает сам redemption. Уже выполнен
  (кто-то подтвердил руками) — операция помечается выполненной. Отклонён (ETH
  вернулись зрителю) — `CANCELED_EXTERNALLY`, «Отменены на Twitch» и отдельное
  предупреждение в панели. GTA$ автоматически **не** списываются; забрать их
  можно только ручной корректировкой (`POST /api/admin/economy/adjust`, кнопки
  в панели для этого нет).
* **Упала транзакция в базе.** Ничего не начислено и не подтверждено; событие
  остаётся необработанным в `eventsub_events` и его подберёт общий повтор
  ([§7](#7-eventsub-быстрый-ответ)).

Повторная доставка одного и того же redemption второй раз не начислит: один
redemption — не больше одного начисления, это уникальный ключ в базе, и он
переживает сброс Redis.

### Проверка: зритель A и зритель B

Нужны два разных Twitch-аккаунта, у каждого не меньше 500 ETH, каждый на своём
устройстве или в своём профиле браузера и **не в домашней сети** (см. ниже).
Стрим идёт, телефон стримера шлёт GPS, приём точек открыт, в панели GTA DOLLAR
ECONOMY «ОПЛАТА В GTA$», награда обмена есть и включена. Цифры — для точки в
1,4 км при настройках по умолчанию; для другой точки подставь цену с карточки.

1. A открывает карту: «Чтобы использовать GTA$, подключите Twitch» →
   **ПОДКЛЮЧИТЬ** → разрешить. В углу карты `GTA$ 0`. B — так же.
2. B выбирает точку: «Цена: GTA$ 1 500», «Ваш баланс: GTA$ 0», кнопка
   неактивна, «Не хватает GTA$ 1 500» и **+ ПОПОЛНИТЬ**. Диалог показывает
   «Обмен ETH на GTA DOLLAR» и `500 ETH → 5 000 GTA$` и не пытается открыть
   награды сам.
3. A в наградах канала Twitch активирует «Обмен ETH на GTA DOLLAR». Через
   секунды, без перезагрузки: у A `+5 000 GTA$` и `GTA$ 5 000`, у B
   по-прежнему `GTA$ 0`. В очереди запросов наград redemption выполнен.
4. A выбирает точку и нажимает **ОТПРАВИТЬ СТРИМЕРА — GTA$ 1 500**: точка
   активна в OBS и на телефоне, у A `GTA$ 3 500`. Баллы канала на этом шаге не
   списываются.
5. Стример на телефоне: **НЕ МОГУ** → ещё раз. Задание отменено, у A
   «Возврат за задание» и снова `GTA$ 5 000`; ETH не вернулись.
6. B активирует награду обмена: у B `GTA$ 5 000`, у A без изменений.
7. B покупает точку: `GTA$ 3 500`. Стример нажимает **ЗАВЕРШИТЬ** → ещё раз:
   у B остаётся `GTA$ 3 500`.
8. A и B обновляют страницу канала — балансы те же: `GTA$ 5 000` и
   `GTA$ 3 500`.
9. `/admin` → GTA DOLLAR ECONOMY: «Выпущено» 10 000, «Потрачено» 3 000,
   «Возвращено» 1 500, «В обращении» 8 500, «Кошельков» 2, журнал сходится,
   «Ждут FULFILLED» 0, «Сбой FULFILLED» 0, «Отменены на Twitch» 0.

### Новый оверлей — новый zip в Hosted Test

В Hosted Test страницы расширения Twitch раздаёт со своего CDN из
**загруженного zip**, а не с нашего сервера. `docker compose up -d --build`
обновляет только бандл на `https://gudinigta6.duckdns.org`, то есть Local Test.
Пока в версии старый zip, у зрителей старая карточка «награда за точку», а в
режиме `gta_dollar` она купить точку не может: сервер отвечает `payment_mode`.

Порядок:

1. Пока новый zip не загружен — `WAYPOINT_PAYMENT_MODE=channel_points_reward`
   в `.env`. Без этой строки после ближайшего пересоздания api уже будет
   `gta_dollar`. Награда обмена при старте api появится на Twitch и в этом
   режиме (см. откат выше).
2. Собрать `web/twitch-extension.zip` с публичным адресом
   ([§8](#8-сборка-расширения)).
3. Console → Extensions → расширение → версия → Files → загрузить zip. Если
   версия в Hosted Test и загрузка закрыта — Move to Local Test, загрузить,
   снова Move to Hosted Test.
4. `WAYPOINT_PAYMENT_MODE=gta_dollar`, `docker compose up -d --force-recreate api`.
5. Зрителям обновить страницу канала (`Ctrl+Shift+R`).

### Из домашней сети Hosted-расширение до API не достаёт

В Hosted Test оверлей загружен с `*.ext-twitch.tv`, а API зовёт на
`https://gudinigta6.duckdns.org`. На этом компьютере `hosts` указывает этот
адрес на `192.168.1.68` — иначе из дома он не открывается вовсе, роутер не
умеет NAT hairpin ([§2](#2-порты-в-роутере)). Chrome (Local Network Access) не
пускает страницу из интернета в локальную сеть, а разрешить это iframe Twitch
не может: запросы к API и сокет блокируются. На остальных домашних устройствах
записи в `hosts` нет, запрос уходит на внешний IP и упирается в тот же hairpin.
Итог одинаковый: ни баланса, ни цены, ни покупки.

Поэтому тестовые зрители — **в другой сети**: телефон на мобильном интернете с
выключенным Wi-Fi или друг у себя дома. Удалять запись из `hosts` бесполезно.
`/admin` и OBS на этом компьютере это не касается: они открываются напрямую, а
не из iframe Twitch.
